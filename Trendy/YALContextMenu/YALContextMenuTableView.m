...
- (void)show:(BOOL)show
  ...
  completion:(completionBlock)completed {
    ...
    UIView *contexticon = [cell animatedcontext];
    ...
    switch (direction) {
        case right: {
            ...
            [self setAnchorPoint:CGPointMake(1, 0.5) forView:contexticon];
        ...
        case top: {
            ...
            [self setAnchorPoint:CGPointMake(0.5, 0.0) forView:contexticon];
        ...
        case bottom: {
            ...
            [self setAnchorPoint:CGPointMake(0.5, 1) forView:contexticon];
            rotationAndPerspectiveTransform = CATransform3DRotate(rotationAndPerspectiveTransform, DEGREES_TO_RADIANS(rotation), 1.0f, 0.0f, 0.0f);
            break;
        }
    ...
    if (animated) {
        [UIView animateWithDuration:self.animationDuration animations:^{
            ...
            contexticon.layer.transform = rotationAndPerspectiveTransform;
             content.alpha = show;
         } completion:^(BOOL finished) {
             if (completed) {
                 completed(finished);
...
- (void)resetAnimatedIconForCell:(UITableViewCell<YALContextMenuCell> *) cell{
    ...
    [icon layoutIfNeeded];
    UIView *contexticon = [cell animatedcontext];
    contexticon.layer.anchorPoint = defaultViewAnchorPoint;
    contexticon.layer.transform = CATransform3DIdentity;
    [contexticon layoutIfNeeded];
}
