...
//            UIImage* image = [self imageFromBundle:@"slider-default-trackCrossedOver"];
            ...
            image = [image resizableImageWithCapInsets:UIEdgeInsetsMake(0.0, 5.0, 0.0, 3.0)];
...
//            UIImage *image = [self imageFromBundle:@"slider-default-trackCrossedOver"];
            UIImage* image = [UIImage imageNamed:@"slider-default-trackCrossedOver"];
            image = [image resizableImageWithCapInsets:UIEdgeInsetsMake(0.0, 2.0, 0.0, 8.0)];
            _trackBackgroundImage = image;
        }
    }
...
- (UIImage *)lowerHandleImageNormal
{
    if(_lowerHandleImageNormal==nil)
    {
        if(IS_PRE_IOS7())
        {
            UIImage* image = [UIImage imageNamed:@"knob"];
...
-(void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    [self sendActionsForControlEvents:UIControlEventValueChanged];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"ENDSLIDER" object:nil];
}
